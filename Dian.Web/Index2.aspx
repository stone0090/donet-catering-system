<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Index2.aspx.cs" Inherits="Dian.Web.Index2" %>
...
<body>
    ...
    <form id="form1" runat="server">
                                            ...
                                            <div class="am-u-sm-8 am-list-main">
                                                ...
                                                <div style="margin-top: 10px;">
                                                    ...
                                                    <span class="am-fr">
                                                        ...
                                                        <button type="button" class="am-btn am-btn-warning am-btn-xs js-modal-open" data-am-modal="{target: '#divRemark', width: 260, height: 450}" onclick="loadRemark('<%# Eval("FOOD_ID") %>');">备注</button>
                                                    </span>
                                                </div>
                                            </div>
                                            <input type="hidden" id="<%# "hFoodRemark" + Eval("FOOD_ID").ToString() %>" />
                                            <input type="hidden" id="<%# "hFoodDetail" + Eval("FOOD_ID").ToString() %>" />
                                        </li>
                                    </ItemTemplate>
                                </asp:Repeater>
        ...
        <!-- “已下单的数据”和“未下单的数据”(json格式) -->        
        <input type="hidden" id="hFinishOrderData" runat="server" />
        <input type="hidden" id="hUnfinishOrderData" runat="server" />        
    ...
    <script type="text/javascript">
        ...
        //页面初始化，如果当前桌还未吃完，则加载已点的菜单。
        $(function () {
            var strFinishOrderData = $('#<%= this.hFinishOrderData.ClientID %>').val();
            if (!$.isEmpty(strFinishOrderData)) {
                //TODO，初始化页面信息
            }
        });
        ...
        function clearCart() {
            ...
            $('input[id^=hFoodRemark]').each(function () {
                $(this).val('');
            });
            recal();
        }
        ...
        function loadRemark(id) {
            ...
            if (!$.isEmpty(strRemark)) {
                ...
                if (!$.isEmpty(oRemark.detail))
                    $('#remarkFoodRemark').val(oRemark.detail);
            }
        }
        ...
        $('#divRemark').on('closed.modal.amui', function () {
            ...
            oRemark.detail = $('#remarkFoodRemark').val();
